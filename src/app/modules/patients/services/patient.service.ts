...
export class PatientService {
  ...
  ): Observable<ApiResponse<InitiateRegistrationResponse>> {
    const body = { ...patient, company_id: this.config.companyId };
    return this.http.post<ApiResponse<InitiateRegistrationResponse>>(
  ...
  ): Observable<ApiResponse<ConfirmRegistrationResponse>> {
    return this.http.post<ApiResponse<ConfirmRegistrationResponse>>(
  ...
  getByDocument(
    ...
    docNumber: string,
    companyId?: number,
  ): Observable<ApiResponse<ShowPatientResponse>> {
    let params = new HttpParams();
    const id = companyId ?? this.config.companyId;
    if (id) {
      params = params.set("company_id", id.toString());
    }
    return this.http.get<ApiResponse<ShowPatientResponse>>(
      `${this.baseUrl}/${docType}/${docNumber}`,
      { params },
    );
  }
  ...
  disableEnable(
    data: DisableEnablePatientRequest,
  ): Observable<ApiResponse<Patient>> {
    return this.http.post<ApiResponse<Patient>>(
      `${this.baseUrl}/disable-enable`,
      data,
    );
  }

  sync(data: SyncPatientRequest): Observable<ApiResponse<SyncPatientResponse>> {
    return this.http.post<ApiResponse<SyncPatientResponse>>(
      `${this.baseUrl}/sync`,
      data,
    );
  }
  ...
  checkExistence(
    data: CheckPatientExistenceRequest,
  ): Observable<ApiResponse<CheckPatientExistenceResponse>> {
    const body = { ...data, company_id: this.config.companyId };
    return this.http.post<ApiResponse<CheckPatientExistenceResponse>>(
      `${this.baseUrl}/check-existence`,
      body,
    );
  }
}
